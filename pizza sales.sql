...
select pizzas.size, count(order_details.order_details_id) as order_count from pizzas join order_details on pizzas.pizza_id = order_details.pizza_id group by size order by order_count desc limit 1;
...
SELECT 
    ROUND(AVG(quantity), 0) as pizza_orders_per_day
FROM
    (SELECT 
    ...
    GROUP BY orders.order_date) AS sum_order_quantity;
